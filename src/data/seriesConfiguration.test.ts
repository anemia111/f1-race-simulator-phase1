...
const editableSeries = seriesPackageById.get('super-formula')!
...
describe('series configuration import and export', () => {
  it('round-trips a validated versioned JSON backup', () => {
    const teams = editableSeries.teams.map((team, index) => ({
      ...team,
      machine: { ...team.machine },
      name: index === 0 ? 'Edited Team' : team.name,
    }))
    const drivers = editableSeries.drivers.map((driver, index) => ({
      ...driver,
    ...
    const restored = importSeriesConfigurationBackup(source, editableSeries)
    ...
    expect(restored.rules).toEqual(editableSeries.rules)
    expect(restored.calendar).toEqual(editableSeries.calendar)
  })

  it('round-trips a historical pool identity in a target-owned F1 seat', () => {
    const outgoing = f1.drivers[0]
    const poolDriver = driverPool2026.find(
      (driver) => driver.id === 'kush_maini',
    )!
    const incoming = seatedDriverFrom(poolDriver, {
    ...
    const transferredField = f1.drivers.map((driver) =>
      driver.id === outgoing.id ? incoming : driver,
    )
    const source = exportSeriesConfigurationBackup(
      f1,
      f1.teams,
    ...
    const restored = importSeriesConfigurationBackup(source, f1)
    const restoredIncoming = restored.drivers.find(
      (driver) => driver.id === poolDriver.id,
    )!

    expect(restoredIncoming.teamId).toBe(outgoing.teamId)
    expect(restoredIncoming.carNumber).toBe(outgoing.carNumber)
    expect(restoredIncoming.skills).toEqual(incoming.skills)
  })
  ...
  it('rejects a backup for a different category', () => {
    ...
    expect(() => importSeriesConfigurationBackup(source, editableSeries)).toThrow(
      SeriesConfigurationValidationError,
    )
  })
  ...
  it('imports validated rule edits and rejects broken event relations', () => {
    const backup = JSON.parse(
      exportSeriesConfigurationBackup(
        editableSeries,
        editableSeries.teams,
        editableSeries.drivers,
        '2026-07-19T00:00:00.000Z',
      ),
    )
    ...
    backup.calendar.pop()
    expect(() =>
      importSeriesConfigurationBackup(JSON.stringify(backup), editableSeries),
    ).toThrow(/calendar is missing ids/i)
  })

  it('strips the retired base-lap multiplier from an older backup', () => {
    const backup = JSON.parse(
      exportSeriesConfigurationBackup(
        editableSeries,
        editableSeries.teams,
    ...
    backup.rules.baseLapTimeMultiplier = 1.04
    ...
    expect(imported.rules).not.toHaveProperty('baseLapTimeMultiplier')
    expect(imported.migrationHistory).toContain(
      'removed legacy baseLapTimeMultiplier',
    )
  })

  it('fails closed for corrupted browser storage', () => {
    expect(parsePersistedSeriesConfiguration('{bad json', editableSeries)).toBeNull()
    expect(
      parsePersistedSeriesConfiguration(
        JSON.stringify({ saveVersion: 99, seriesId: 'super-formula' }),
        editableSeries,
      ),
    ).toBeNull()
  })
  ...
  it('round-trips driver CSV including quoted text and grouped abilities', () => {
    const edited = editableSeries.drivers.map((driver, index) => ({
      ...driver,
    ...
    const imported = importDriverCsv(csv, editableSeries, editableSeries.drivers, editableSeries.teams)
    ...
    expect(imported).toHaveLength(editableSeries.carCount)
  })

  it('rejects duplicate car numbers in driver CSV', () => {
    const rows = exportDriverCsv(editableSeries.drivers).trim().split(/\r?\n/)
    ...
    expect(() =>
      importDriverCsv(rows.join('\n'), editableSeries, editableSeries.drivers, editableSeries.teams),
    ).toThrow(/duplicate car numbers/i)
  })

  it('round-trips machine CSV and preserves exact team ids', () => {
    const csv = exportTeamCsv(editableSeries.teams)
    const imported = importTeamCsv(csv, editableSeries, editableSeries.teams)

    expect(imported.map((team) => team.id)).toEqual(
      editableSeries.teams.map((team) => team.id),
    )
    expect(imported[0].machine.puOutput).toBeCloseTo(
      editableSeries.teams[0].machine.puOutput,
      4,
    )
  })

  it('equalises performance without changing team identity', () => {
    const equalised = equalizeMachinePerformance(editableSeries.teams)
